...
def cfgStats(m:Method): CfgStats = {
  // force node type to CfgNode so .code / .lineNumber are available
  val nodes: List[CfgNode] = m.cfgNode.l.collect { case n: CfgNode => n }.distinct
  val nCount = nodes.size
  val edges  = nodes.flatMap(n => n.cfgNext.l.collect{ case nn: CfgNode => (n, nn) })

  // back-edge heuristic: successor lineNumber < current lineNumber
  def ln(x: CfgNode) = x.lineNumber.map(_.toInt)
  val backEdges = edges.count{ case (a,b) => (for {la <- ln(a); lb <- ln(b)} yield lb < la).getOrElse(false) }

  // build acyclic adjacency by skipping "back" edges
  val dag = edges
    .filterNot{ case (a,b) => (for {la <- ln(a); lb <- ln(b)} yield lb < la).getOrElse(false) }
    .groupBy(_._1).view.mapValues(_.map(_._2).toSet).toMap

  // longest path in DAG (DFS with memo)
  val memo = mutable.Map.empty[Long, Int]
  def id(n: CfgNode) = n.id
  def dfs(u: CfgNode): Int =
    memo.getOrElseUpdate(id(u), dag.getOrElse(u, Set.empty).map(v => 1 + dfs(v)).foldLeft(0)(Math.max))

  val longest = nodes.map(dfs).foldLeft(0)(Math.max)

  // basic segments ~ number of starts of straight-line runs (branch/merge heuristics)
  val isBranchNode: CfgNode => Boolean = n => {
    val code = Option(n.code).getOrElse("").trim.toLowerCase
    code.startsWith("if") || code.startsWith("switch") || code.startsWith("case") || code.startsWith("default")
  }
  val outDeg = edges.groupBy(_._1).view.mapValues(_.size).toMap
  val inDeg  = edges.groupBy(_._2).view.mapValues(_.size).toMap
  val segments = nodes.count { n =>
    val o = outDeg.getOrElse(n,0); val i = inDeg.getOrElse(n,0)
    (i != 1) || isBranchNode(n)
  }
  CfgStats(nCount, edges.size, backEdges, longest, segments)
}
